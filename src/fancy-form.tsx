          ...
          <div className="grid grid-cols-2 gap-4">
            {Object.entries(preferences).map(([key, value]) => (
              <Toggle
                key={key}
                pressed={value}
                onPressedChange={() => togglePreference(key as keyof typeof preferences)}
                className={`w-full h-16 font-semibold border-2 ${
                  value
                    ? `text-white border-transparent ${
                        key === 'analytical' ? 'bg-blue-500 hover:bg-blue-600 data-[state=on]:bg-blue-500 data-[state=on]:hover:bg-blue-600' :
                        key === 'conceptual' ? 'bg-yellow-500 hover:bg-yellow-600 data-[state=on]:bg-yellow-500 data-[state=on]:hover:bg-yellow-600' :
                        key === 'social' ? 'bg-red-500 hover:bg-red-600 data-[state=on]:bg-red-500 data-[state=on]:hover:bg-red-600' :
                        'bg-green-500 hover:bg-green-600 data-[state=on]:bg-green-500 data-[state=on]:hover:bg-green-600'
                      }`
                    : `bg-white ${
                        key === 'analytical' ? 'text-blue-500 border-blue-500 hover:bg-blue-100' :
                        key === 'conceptual' ? 'text-yellow-500 border-yellow-500 hover:bg-yellow-100' :
                        key === 'social' ? 'text-red-500 border-red-500 hover:bg-red-100' :
          ...
          {Object.entries(traits).map(([key, value]) => (
            <div key={key} className="space-y-2">
              ...
              <div className="flex gap-2">
                <Button
                  onClick={() => updateTrait(key as keyof typeof traits, 'first-third')}
                ...
                <Button
                  onClick={() => updateTrait(key as keyof typeof traits, 'second-third')}
                ...
                <Button
                  onClick={() => updateTrait(key as keyof typeof traits, 'third-third')}
